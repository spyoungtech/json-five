...
def loads(s, *args, loader=None, **kwargs):
    ...
    return loader.load(model, environment)
...
class DefaultLoader:
    ...
    @to_python(JSONText)
    def json_model_to_python(self, node, env):
        logger.debug('json_model_to_python evaluating node %r', node)
        return self.load(node.value, env)

    @to_python(JSONObject)
    def json_object_to_python(self, node, env):
        logger.debug('json_object_to_python evaluating node %r', node)
        d = {}
        for key_value_pair in node.key_value_pairs:
            key = self.load(key_value_pair.key, env)
            value = self.load(key_value_pair.value, env)
            d[key] = value
    ...
    @to_python(JSONArray)
    def json_array_to_python(self, node, env):
        logger.debug('json_array_to_python evaluating node %r', node)
        return [self.load(value, env) for value in node.values]

    @to_python(Identifier)
    def identifier_to_python(self, node, env):
    ...
    def number_to_python(self, node, env):
    ...
    @to_python(Integer)
    def integer_to_python(self, node, env):
        if env.parse_int:
    ...
    @to_python(Float)
    def float_to_python(self, node, env):
        if env.parse_float:
